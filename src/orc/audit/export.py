...
def _render_readme(m: ExportManifest) -> str:
    return (
        ...
        + (
            ...
            "\n"
            "Extract and replay in an isolated environment — the example below\n"
            "uses ORC_HOME so the replay does not touch your existing Orc data:\n"
            "\n"
            "```\n"
            "tar xzf <this-bundle>.tar.gz\n"
            "\n"
            "# Mount the extracted workspace under a fresh ORC_HOME using the\n"
            "# ORIGINAL workspace name from manifest.json. The replay engine\n"
            "# resolves traces by the workspace name recorded inside each trace\n"
            "# JSON, so this name must match — renaming will break replay.\n"
            f"WS={m.workspace}\n"
            "mkdir -p ./orc-replay-home/workspaces/$WS\n"
            "cp -R workspace/* ./orc-replay-home/workspaces/$WS/\n"
            "\n"
            "ORC_HOME=$(pwd)/orc-replay-home orc replay <run_id>\n"
            "```\n"
            "\n"
